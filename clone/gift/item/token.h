...
{
	if( i >= 28 )	return HIW"完美"NOR;
        if( i >= 20 )	return HIW"极高"NOR;
...
void token_reload()
{
	...
	{			
		if (this_player()->query("token/"+query("id")+"/parry")) 
		{
			i=this_player()->query("token/"+query("id")+"/parry");
                        set("armor_prop/parry",i);
			str="防御力";
		}	
		if (this_player()->query("token/"+query("id")+"/dodge"))
		{
			i=this_player()->query("token/"+query("id")+"/dodge");
                        set("armor_prop/dodge",i);
			str="躲避率";
		}
		if (this_player()->query("token/"+query("id")+"/armor_vs_force"))
		{
			i=this_player()->query("token/"+query("id")+"/armor_vs_force");
