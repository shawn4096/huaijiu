...
int do_imbue(string arg)
{
        ...
        if( !me->query("imbue_reward")){
        	...
        	if( me->query("combat_exp")/1000000 < (int)me->query("user_weapon_imbued") ){
                	return notify_fail("你还是好好练功，过段时间再来吧。\n");
        	}
...
int accept_object(object who, object ob)
{
	...
	if(ob->query("dazao_award") && ob->id("weilan's hammer"))
	{
		command("ah");
		command("thank "+getuid(who));
		command("whisper "+getuid(who)+" 我也不好什么都不表示，就再帮你打造一次吧。");
		who->add("imbue_reward",1);
            destruct(ob);
            log_file( "quest/hammer",
		sprintf("%s %s(%s)提交韦兰之锤一柄，尚余额外打造次数：%d\n",
			ctime(time())[4..15],
			who->query("name"),
			who->query("id"),
			who->query("imbue_reward")
		)
	);
