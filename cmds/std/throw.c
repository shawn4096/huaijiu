...
private int throw_result(object me, object victim, object ob)
{
	...
	if (random(ap+dp) > dp) {
		...
		victim->receive_wound("qi", me->query_str() * ob->query("throw/damage"), me);
		victim->receive_damage("qi", (ob->query_weight()/5000+me->query_str()) * ob->query("throw/damage"), me);
		if(ob->query("embed")){
