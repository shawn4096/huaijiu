...
void get_desc(string desc)
{
	...
	input_to("get_choice", ch, 1);
}

void get_choice(string arg, mapping ch, int i)
{
	if (arg == "") return;
	if (arg == ".") {
		vote=([
...
void show_vote()
{
	...
	if (vote["total"]) for (i = 1; i <= vote["num"]; i++) {
		item = vote["choice"][""+i];
		write(
			sprintf("%-30s%4d%7.2f%%\n", "选择 " HIG + item + NOR " 的票数：",
				vote["votes"][item],
...
int main(object me, string arg)
{
	...
	if (sscanf(arg, "%d", i)) {
		...
		if (exp >= 1000000) {vote["votes"][item]++; vote["total"]++;}
		if (exp >= 2000000) {vote["votes"][item]++; vote["total"]++;}
		if (exp >= 4000000) {vote["votes"][item]++; vote["total"]++;}
		if (exp >= 8000000) {vote["votes"][item]++; vote["total"]++;}
		me->set("votes", t);
		if (!(vote["total"] % 5))
			save();
		show_vote();
