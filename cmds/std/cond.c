...
//		case "bonze_jail"	: name = "少林监狱"; break;
		...
		case "dmd_time"  	: name = "勇闯夺命岛"; break;                
		case "sjsz_time"	: name = "试剑山庄探险"; break;                
...
├────────────────────────┤\n";
		...
		for ( i=0; i<sizeof(cons); i++ )
		{
			...
			if (values(cons)[i] < 3)
				cont = chinese_number(values(cons)[i]*20)+"秒";
			else
				cont = chinese_number(values(cons)[i]/3)+"分";
