...
int main(object me, string arg)
{
	...
	if (ob->query("no_drop") || userp(me) && (ob->query("no_get") || ob->query("equipped") || ob->query("unique")))
	...
	me->start_busy(cost);
	call_out( "compelete_beg", cost, me, victim, ob, sp, dp);
...
private void compelete_beg(object me, object victim, object ob, int sp, int dp)
{
	...
	if( !ob || environment(ob) != victim || ob->query("imbued") ) {
