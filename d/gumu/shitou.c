...
int do_lingwu(string arg)
{
          ...
          if (!(fam = me->query("family")) || fam["family_name"] != "古墓派")
               return notify_fail("你不是古墓传人，如何能领悟古墓武功？\n");
          if (! arg ) return 0;
          if ( arg != "parry" && arg != "sword" && arg != "cuff"
           &&  arg != "strike" && arg != "force" && arg != "throwing" && arg != "dodge" && arg !="hand" )
               return notify_fail("你无法从这里领悟其他的武功。\n");
          lvl = me->query_skill( arg, 1);
          if ( lvl < 101 ) return notify_fail("你的"+to_chinese(arg)+"太低，无法在此领悟。\n");
          if ( lvl > 120 ) return notify_fail("你已经无法从这里领悟到什么了。\n");
          if (me->query("jing") < 40)
