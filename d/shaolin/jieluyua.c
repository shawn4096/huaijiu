...
void create()
{
	...
	set("long", @LONG
整个大殿门户紧闭，密不透风。即便是在白昼，也犹如黑夜一般，每隔
几步的墙上，点燃着几枝牛油巨烛。殿上供奉着降魔韦陀下方摆着一排排水
牛皮鞭、穿孔竹板及狼牙棒等执法刑杖。四周站满了怀抱戒刀的执法僧兵。
他们正用威严的眼神打量着你，令你不禁心生惧意。
LONG
	);
...
{	
	...
	if (interactive(ob = this_player()))
	{
		...
		if (!ob->query("guilty") && ob->query("family/family_name") == "少林派")
			return;
...
void beating1(object ob)
{
	...
	new("/d/shaolin/obj/liao_kao")->move(ob);
        ob->unconcious();
	ob->move("/d/shaolin/houdian");
