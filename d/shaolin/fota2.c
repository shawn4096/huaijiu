...
int do_canchan(string arg)
{
 ...
 me=this_player();
 if (arg!="zuo"&&arg!="圣座") return notify_fail("你要对什么参悟禅理？\n");
 message("vision",
         me->name()+"对着大师的圣座冥思参禅。\n",
         environment(me), ({me}) );
  ...
  if(me->query_temp("shen_said"))
   {
    ...
    write("你身形一纵，十指如钩，在禅台边上一抓要借力跃起。\n");
     message("vision",
                me->name()+"身形一纵，十指如钩，在禅台边上一抓要借力纵起。\n",
               environment(me), ({me}) );
    if((int)me->query_skill("claw",1)<30)
   {
     write("你手指与台边差了寸许，没有抓到。啊.....。\n");
        message("vision",
        me->name()+"手指与台边还有一定距离，没有抓到。\n",
        environment(me), ({me}) );
        message("vision",
        me->name()+"一声大叫，一个倒栽葱摔将下来。\n",
        environment(me), ({me}) );
        me->unconcious();   
        return 1;    
   }
...
int do_chuzhang(string arg)      
  {
    ...
    if ((int)me->query_temp("marks/拍")) {
              if(arg!="pai"&&arg!="牌") return notify_fail("你要哪出掌？\n");
