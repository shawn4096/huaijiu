...
int do_du(string arg)
{
        ...
        if ( jylevel >= (me->query_skill("shenghuo-shengong", 1)+ 10))
        ...
        if( !me->query("jiuyang/read")){
          ...
          log_file("quest/jiuyang",sprintf("%s %s(%s)成功得到「九阳真经」，并开始研读九阳神功，此时九阳神功等级是：%s。\n",
                ctime(time())[4..19],me->name(1),capitalize(me->query("id")), chinese_number(jylevel)));
