...
string look_sl()
{
    object me, room;
    me = this_player();
    if (!(room = find_object("/d/shaolin/guangchang1")))
          room = load_object("/d/shaolin/guangchang1");
...
void create()
{
        set("short", "观胜峰");
        set("long", @LONG
站在这里，凌于瀑布之上，景色又是一番不同，处在一片山从之中，
西北是绝顶，西面是嵩山的各个山峰，南面遥遥相望的便是地处少室山
的嵩山少林寺(shaolin)，身处山中而不见归路。路边到处可见一堆堆
的乱石。
LONG);
        set("exits", ([
