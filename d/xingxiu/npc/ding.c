...
void greeting(object me)
{
	...
	if(!myfam = (mapping)me->query("family")) 
		...
		log_file( "job/xx_job5",sprintf("%s %s(%s)做过%d次任务离开星宿，减去经验：%d。\n",
...
//       || partner->query("env/invisibility") 
	 ...
	 || partner == me ){
	    ...
	    log_file( "job/xx_job5",sprintf("%s %s(%s)第%d次任务开始，总计e：%d，p：%d；exp：%d\n",
		ctime(time())[4..19],me->name(),me->query("id"),me->query("xx_job5")+1,me->query("xx_jobe"),me->query("xx_jobp")
		,me->query("combat_exp")));
	    log_file( "job/xx_job5",sprintf("%s %s(%s)第%d次任务搭档，总计e：%d，p：%d；exp：%d\n",
...
int give_reward(object me)
{
	...
	log_file( "job/xx_job5",sprintf("%s %s(%s)第%d次任务结束，经验：%d，潜能：%d；总计e：%d，p：%d；exp：%d\n",
		ctime(time())[4..19],me->name(),me->query("id"),me->query("xx_job5"),e,p,me->query("xx_jobe"),me->query("xx_jobp")
		,me->query("combat_exp")));
	log_file( "job/xx_job5",sprintf("%s %s(%s)第%d次搭档结束，经验：%d，潜能：%d；总计e：%d，p：%d；exp：%d\n",
...
string ask_fail()
{
	...
	log_file( "job/xx_job5",sprintf("%s %s(%s)放弃了第%d次任务，减少了经验%d。\n",
...
string ask_job5_times()
{
	...
	return ""+ob->name()+"，你帮我派杀死了"+HIY+CHINESE_D->chinese_number((int)ob->query("xx_job5"))+NOR+"个叛徒！";
}
...
string ask_job4()
{
	...
	if( me->query_condition("job_busy")) return "你还忙着干别的呢，呆会再来吧。";
	if(me->query_condition("gb_mission") || me->query_condition("gb_busy"))
		return "老仙最讨厌臭叫化子，先扔掉你的丐帮帮务再说！";
