...
{   
   ...
   if((!me->query_temp("xx_zuji"))&& me->query("combat_exp")>100000)gain=gain/4;
   ...
   tell_object(me, YEL "你的经验和潜能各增加了"
	 +chinese_number(gain)+"点！\n" NOR);
   log_file( "job/xx_job4",sprintf("%s %s(%s)做第%d次星宿抓虫，得到：%d点EXP和POT。\n",
   ctime(time())[4..19],me->name(), getuid(me), me->query("xx_job4"),gain));
   return;      
}
...
void die()
{
	...
	{
		ob = new("/d/xingxiu/npc/kanshou");
		ob->move("/d/xingxiu/rukou");
	}
	::die();
}