...
//                        "归元丹" : (: ask_dan :),
                        ...
                        "图解" : (: ask_book2 :),
                        "武学" : (: ask_learn :),
//                 "武学" : "血滴子说了，这个任务有bug，劝你们别做了, 免的被purge。\n",
                        "复命" : (: ask_fuming :), 
                                                "放弃" : (: ask_fangqi :),
                ]));
        set("dan_count", 2);
...
string ask_9dan()
{
        ...
        if (present("huizhuan dan", ob))
                return RANK_D->query_respect(ob) + "你现在身上不是有吗，怎么又来要了？真是贪得无厌！";
        if (ob->query_temp("mr/get_9dan")>0)
                return RANK_D->query_respect(ob) + "你刚才要过，怎么又来要了？真是贪得无厌！";

        
        if (query("dan") < 1) return "对不起，药已经发完了，新药尚未配制出来。";
 
        obj = new("/d/mr/obj/9dan");
        obj->move(ob);
        ob->set_temp("mr/get_9dan",1);

        add("dan", -1);

        message_vision("$N获得一颗"HIY"九天回转丹"NOR"。\n", ob);
        return "好吧，此药来之不易，不倒万不得已之时切勿服用。";
}
