...
void create()
{
       set_name(HIW"卷帛"NOR, ({ "paper" }) );
       set_weight(100);
       if( clonep() )
	       set_default_object(__FILE__);
      else {
	       set("unit", "张");
	       set("long", "这是一张微微发黄的卷帛。\n");
	       set("value", 0);
	       set("no_get",1);
	       set("no_drop",1);
	       set("material", "paper");
...
int do_report(string arg)
{
	...
	write("这是一张发黄的卷帛，上面写着：偷学"+ HIW+chinese_number(me->query_temp("skill_lvl"))+NOR+"级"+HIY+me->query_temp("skill_target")+HIR+"("+me->query_temp("skill_id")+")。"+NOR+"\n");
	return 1;
}
