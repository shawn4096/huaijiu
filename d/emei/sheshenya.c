...
int do_jump(string arg)
{
        ...
        if (me->query("emeijiuyin/failed")>=3)
		...
		me->move(__DIR__"bankong");
                if(local[2] > 13 && local[2] < 16 ) {
                        if (random(me->query("kar")) > 8 && (int)me->query_dex() > 32)
                                call_out("do_wait", 2, me);
			else if ( me->query("hanshui_food") && random(3) == 1  )
                                call_out("do_wait", 2, me);
			else call_out("do_wait1", 2, me);
			return 1;
		}
