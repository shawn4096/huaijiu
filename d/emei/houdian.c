...
void reset()
{
    object me, here=this_object();
    if ( here->query("fighting") != 1 ) {
        if ( here->query("count") != 1 ) 
            here->set("count", random(4) );
        if ( me = LOGIN_D->find_body("keepon") )
            tell_object(me, "hi, i am reseting in houdian.\n" + here->query("count"));
...
void test(object here)
{
    ...
    list = filter_array(users(), (: $1->query("family/family_name") == "峨嵋派" :));
