...
string ask_finish()
{
	...
	switch(me->query_temp("changle/done")){
		case 1:
                   TASKREWARD_D->get_reward(me,"长乐帮",0,0,0,-4,0,0,0);
			msg = "既然已经知道了是"+msg+"所为，哼，这个梁子算结上了。";
			me->add("job_time/长乐帮", -1);
			break;
		case 2:
                   TASKREWARD_D->get_reward(me,"长乐帮",0,0,0,1,0,0,0);

			msg = "哎，可惜不能手刃此奸贼，但"+msg+"和我帮的梁子算结上了。";
			me->add("job_time/长乐帮", -1);
			break;
		case 3:
                   TASKREWARD_D->get_reward(me,"长乐帮",0,0,0,5,0,0,0);
			gold = 3 +random(3);
...
string ask_fail()
{
	...
	if (!me->query_temp("changle"))
		return "你根本就没有领任务，放弃什么啊？";

	i = me->query_condition("job_busy");
	if ( i > 5 ) i = 5;
		me->apply_condition("job_busy", i );
