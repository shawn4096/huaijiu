...
int do_kanshou()
{
       ...
       if (ob->query_temp("wd_job1"))
        ...
        call_out("give_reward",180+random(60),ob);
        ob->start_busy(1);
        ob->set("job_name", "看守铜鼎");
        call_out("do_attack",90+random(30),ob);
        if(random(10) > 2){
...
int do_attack(object ob)
{
        ...
        exp = ob->query("combat_exp");
        lv = ob->query_skill("dodge",1) - (10+random(20));
        ...
        att->set_name(name, ({ "attacker", ob->query("id")+"'s attacker" }));
...
int give_reward(object ob)
{
        ...
        me = present("yu daiyan",  environment(ob));
        here=environment(ob);
        e = 100 + random(50)+ random(ob->query("wd_job"))/20;
        p = e/5 + random(e/6);
        if(e > 200) e = 200 + random(e-200);
        ob->delete_temp("apply/short");
        ...
        write_file( "/log/job/wd_job1",sprintf("%s %s(%s)看守铜鼎获得了%d点经验，%d点潜能。\n",
...
int do_yan(string arg)
{
        ...
        if (!me->query_temp("yao_get") && !( present("yao dai", me)))
