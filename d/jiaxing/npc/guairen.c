        ...
        if(me->query_temp("oyf/ok")){
        	...
        	//command("say inv="+sizeof(ob));
                for(i=0; i<sizeof(ob); i++) {
        	       ...
        	       log_file("quest/oyf_log", sprintf("当 %s(%s) 于 %s 非法进入ouyang feng处时， %s(%s) 在现场。\n", me->name(1),me->query("id"), ctime(time()), ob[i]->name(1),ob[i]->query("id") ) ); 
        	} 
        } 	
...
void do_teach(object oyf, object me)
{
      ...
      if(me->query("con") < 22){
             ...
             tell_object(me, HIC "\n怪人开始给你讲解蛤蟆功的入门功夫。。。\n" NOR );
             log_file("oyf_hmg", sprintf("%s(%s) 于 %s 成为ouyang feng义子，并学会hmg。\n", me->name(1),me->query("id"), ctime(time()) ) );              
             me->set("oyf/hamagong", 1);   
             me->set_skill("hamagong", 10);        
             add_exp(me);  
             tell_object(me, "你学会了蛤蟆功！\n"); 
             command("chat 哈哈哈哈！"+me->name()+HIC"真是我的怪儿子！"NOR);
        }         
...
void do_leave(object me)
{
      	...
      	message_vision("\n接着$N一步跨了出去。这一步迈得好大，待得第二步跨出，人已在
丈许之外，连跨得十来步，身子早在桑树林後没了。\n\n",oyf);   
      	if(!me->query("oyf_son")){
        	...
        	log_file("quest/oyf_log", sprintf("%s(%s) 于 %s 见到ouyang feng，并得到奖励。\n", me->name(1),me->query("id"), ctime(time()) ) );
        }
