...
*/
	...
	if (clonep()) call_out("remove_ob",1800);
}
...
void init2(object me)
{
	...
	&& file_name(environment()) == "/d/hengshan/baiyunan") {
		if( !me->query_temp("sljob") ) {
			log_file( "job/sljob1", sprintf("%s 少林“救援恒山”"
				+"任务出现BUG。\n", ctime(time())[4..19] ));
		...
		for(i=1;i<=teams;i++) {
			...
			if(ob1 && present(ob1)) {
				...
				&& ob1->query("wxz/quest") < 2) {
					...
					ob1->set("wxz/quest",1);
					log_file("quest/slskill", sprintf("%s %s(%s) 获得学习无相劫指的机会。\n",
						ctime(time())[4..18], ob1->query("name"), ob1->query("id")));
				}
			}
		}
		call_out("remove_obj",1);
		...
		i=random(teams+1);
		ob1=query_temp("team"+i);
		if (ob1 && present(ob1) && random(10)>4 ) {
			...
			log_file( "quest/neili", sprintf("%s %s(%s) 得到六阳正气丹。\n",
...
void die()
{
	...
	log_file( "job/sljob1", sprintf("%s %18s，任务失败。\n", ctime(time())[4..15], 
