...
int valid_enable(string usage) {
	if (userp(this_player()) && this_player()->query("family/family_name") != "古墓派") 
		return notify_fail("你非古墓弟子，不得偷学古墓绝技。\n");
	return (usage == "dodge");
}
...
mapping query_action(object me, object weapon)
{
	...
	out = (stringp(environment(me)->query("ourdoors"))?1:0);
	if(userp(me) && me->query("family/family_name")!="古墓派")
		return ([
