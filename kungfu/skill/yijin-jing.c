...
/*
private void clear_poison(object me)
{
	int i, j;
	mapping poison_name;
	string *poisons;

	if( !mapp(poison_name = me->query_temp("fanzhen/poisons")) )
		return;
	if( !mapp(me->query_conditions_by_type("poison")) )
		return;
	if( !arrayp(poisons = keys(me->query_conditions_by_type("poison"))) )
		return;
	j = sizeof(poison_name);
	if( wizardp(me) )
		tell_object(me, "Poisons size: "+sizeof(poisons)+", name size: "+j+".\n");
	for(i = 0; i< sizeof(poisons); i++ ){
		if( wizardp(me) )
			tell_object(me, "Poison name: "+poisons[i]+", I = "+i+".\n");
		if( !mapp(poison_name[poisons[i]]) ){
			me->clear_condition(poisons[i]);
			if( wizardp(me) )
				tell_object(me, "Jingang -> cleared "+poisons[i]+", I = "+i+".\n");
		}
	}
}
*/
