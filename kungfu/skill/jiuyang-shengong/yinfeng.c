...
{        
        ...
        if( (int)me->query_skill("jiuyang-shengong",1) < 130 )
                return notify_fail("你的九阳神功等级不够，不能施展阴风刀内劲！\n");

        if( (int)me->query_skill("shenghuo-lingfa",1) < 130 )
   ...
   if( random(force)*(me->query_temp("yinfeng/time")+1) > target->query_skill("force")/2
         && me->query("combat_exp")/2 *(me->query_temp("yinfeng/time")+1) > random(target->query("combat_exp"))){
...
//                if(!target->is_killing(me->query("id"))) target->kill_ob(me);
                ...
                if(me->query_temp("yinfeng/time")>2) me->set_temp("yinfeng/time",2);
