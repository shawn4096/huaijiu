...
int perform(object me, object target)
{
        ...
        if((string)me->query_skill_mapped("sword") != "dugu-jiujian" && userp(me))
                  return notify_fail("你现在无法使用「破箭式」！\n");
        ...
        me->start_perform(3,"「破箭式」");
        me->add("jingli", -1000);
        if (random(me->query_int()) > target->query_int()/2){
        msg+= HIG"\n$n只觉得$N每一招中，将自己的暗器克制的毫无还手之余！\n"NOR;
        target->add_temp("apply/throwing",-skill);
        me->set("jiali",0);
...
void remove_effect(object target, int skill)
{
     if (!target) return;
     target->add_temp("apply/throwing",skill);
     message_vision(HIW"$N的暗器运使恢复正常了。", target);
}
