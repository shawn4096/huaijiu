...
mapping query_action(object me, object weapon)
{
        ...
        d = (int)me->query_dex(1) * (random(2)+1);
        p = (int)me->query_int(1) * (random(2)+1);
        ...
        if( me->query_temp("shuangjue/dgb")){
                ...
                return ([
                "action": msg,
                "dodge": me->query("gender")=="女性"?level/5+20:level/5,
                "parry": me->query("gender")=="女性"?level/6+p+d:level/7+p,
                "damage": me->query("gender")=="女性"?level/3+d:level/3+p/2,
                "force": me->query("gender")=="女性"?level+level/2:level+level,
                "damage_type": "挫伤"
                ]);
        }
        ...
        if (level > 400 && userp(me))
                return ([
                        ...
                        "dodge":me->query("gender")=="女性"?level/10+d:level/10+(d+p)/2,
                        "parry": me->query("gender")=="女性"?level/11+(p+d)/3:level/11+(p+d)/3-40,
                        "damage": me->query("gender")=="女性"?level/10+(d+p)/4:level/7+p/5,
                        "force" : me->query("gender")=="女性"?action[j]["force"]+level/10+p:action[j]["force"]+level/10+d/4,
                        "lvl" : action[j]["lvl"],
        ...
        return ([
                ...
                "parry": me->query("gender")=="女性"?level/11+p*3/2:level/11+p*2/3,
                "damage": me->query("gender")=="女性"?level/20+d/2:level/20,
                "force" : me->query("gender")=="女性"?action[j]["force"]:action[j]["force"]/2,
                "lvl" : action[j]["lvl"],
                "damage_type" : action[j]["damage_type"],
        ]);
}
...
mixed hit_ob(object me, object victim, int damage_bonus)
{
        ...
        if(!me->is_fighting(victim)||!weapon) return 0;
        ...
        if( random(20) > 6
         && (int)me->query_skill("stick", 1) >= 200
         && level >= 200
         && living(me)
         && living(victim)
         ...
         && random(b+50) >=120 ){
                ...
                switch(x){
                        ...
                        case 4: msg = HIB"只见慢天棒影一停，$N将「天下无狗」第四变使出，脚在"+ weapon->name() +HIB"一端一踢，"+ weapon->name() +HIB"突然飞向$n的小腹。\n"NOR; break;
                        case 3: msg = HIG"$N使出「天下无狗」第三变，将"+ weapon->name() +HIG"抡圆了，突然直捅$n的眼睛！\n"NOR; break;
                        case 2: msg = HIW"$N闪身后跳，使出「天下无狗」第二变，手中"+ weapon->name() +HIW"猛扫向$n的脚踝。\n"NOR; break;
                        case 1: msg =  HIC"忽然$N急走数步，回身"+ weapon->name() +HIC"斜劈，使出「天下无狗」第一变，戳向$n的肩头。\n"NOR;break;
                        case 0: msg = HIC"$N棒走偏峰，将"+ weapon->name() +HIC"舞成满天棒影，竟然使的是「天下无狗」！\n"NOR; break;
                        default: msg = HIC"$N棒走偏峰，将"+ weapon->name() +HIC"舞成满天棒影，竟然使的是「天下无狗」。\n"NOR; break;
                }
         ...
         && random(20) > 17 ){
                ...
                if (!random(2)){
                        ...
                        victim->recieve_wound("qi",damage/10,me);
                        ...
                        victim->receive_damage("qi",damage/20,me);
                        victim->receive_wound("qi", damage/20,me);
                        p = (int)victim->query("qi")*100/(int)victim->query("max_qi");
                        msg += damage_msg(damage, "劈伤");
                        msg += "( $n"+eff_status_msg(p)+" )\n";
                }
